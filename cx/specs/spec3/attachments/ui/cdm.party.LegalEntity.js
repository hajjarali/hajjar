(function () {
    ...
    return { main: function () {
        ...
        const identity = section(root + "_identity", "Identity", [
            gridField(fid("name"), 12, "Name", "The legal entity's legal name, e.g. BlackRock Investment Management (UK) Limited")
        ]);

        const identEl = viewUtils.buildUIElement(fid("entityIdentifier"), {});
        const identifiers = section(root + "_identifiers", "Identifiers", [
            { id: identEl.id + "_gi", jsontype: "mui.grid", props: { size: 12 }, elements: { [identEl.id]: identEl } }
        ]);

        return { sx: { p: 2, maxWidth: 720, mx: 'auto', minWidth: 320 },
                 elements: { [identity.id]: identity, [identifiers.id]: identifiers } };
    } };
})();