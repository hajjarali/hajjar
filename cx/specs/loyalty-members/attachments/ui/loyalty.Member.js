    ...
    return { main: function () {
        ...
        const bannerSvg = { id: bannerSvgId, jsontype: "ui.svg",
            props: { sx: { position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' },
                     svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 120" preserveAspectRatio="none"><defs><linearGradient id="bannerGrad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#0B1F4B"/><stop offset="100%" stop-color="#5B2A86"/></linearGradient></defs><rect width="400" height="120" fill="url(#bannerGrad)"/></svg>' } };

        const nameEl = viewUtils.buildUIElement(fid("fullName"), { variant: "standard",
            sx: { '& .MuiInputBase-input': { color: '#FFFFFF', fontSize: '1.5rem', fontWeight: 600 },
                  '& .MuiInputLabel-root': { color: 'rgba(255,255,255,0.75)' } } });
        const numberEl = viewUtils.buildUIElement(fid("memberNumber"), { variant: "standard",
            sx: { '& .MuiInputBase-input': { color: '#D4AF37', fontSize: '1.1rem', fontWeight: 600 },
                  '& .MuiInputLabel-root': { color: 'rgba(255,255,255,0.75)' } } });
        const nameColId = bannerId + "_namecol";
        const nameCol = { id: nameColId, jsontype: "mui.box", props: { sx: { flex: 1 } },
            elements: { [nameEl.id]: nameEl, [numberEl.id]: numberEl } };
        const shieldId = bannerId + "_shield";
        const shield = { id: shieldId, jsontype: "ui.svg",
            props: { sx: { width: 56, height: 56, flexShrink: 0 },
        ...
        const bannerContent = { id: bannerContentId, jsontype: "mui.stack",
            props: { direction: "row", spacing: 2, alignItems: "flex-start", sx: { position: 'relative', p: 2 } },
        ...
        const star = { id: starId, jsontype: "mui.icon",
            props: { children: "star", sx: { color: '#D4AF37', fontSize: 32 } } };
        const pointsEl = viewUtils.buildUIElement(fid("pointsBalance"), { variant: "standard",
            sx: { '& .MuiInputBase-input': { color: '#D4AF37', fontSize: '2rem', fontWeight: 700 } } });
        const pointsRowId = root + "_pointsrow";
        const pointsRow = { id: pointsRowId, jsontype: "mui.stack",
            props: { direction: "row", spacing: 1, alignItems: "center", sx: { mb: 2, px: 1 } },
            elements: { [starId]: star, [pointsEl.id]: pointsEl } };
