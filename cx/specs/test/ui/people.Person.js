(function () {
    ...
    return { main: function () {
        ...
        const basics = section(root + "_b", "Name", [
            gridField(fid("firstName"), 6, "First Name"),
            gridField(fid("lastName"), 6, "Last Name")
        ]);
        const contact = section(root + "_c", "Contact", [
            gridField(fid("email"), 6, "Email", "Unique across the register when present."),
            gridField(fid("dateOfBirth"), 6, "Date of Birth")
        ]);
        return { sx: { p: 2, maxWidth: 720, mx: 'auto', minWidth: 320 },
                 elements: { [basics.id]: basics, [contact.id]: contact } };
    } };
})();
